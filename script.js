  ...
  if (isNumeric(val) || val === "." || val === "*-1") {
    if (val === "*-1") {
      ...
      v[insId].displayEquation =
        v[insId].equation.join(" ") + " negative(" + temp + ")";
    } else if (val === ".") {
    ...
    v[insId].displayEquation =
      v[insId].equation.join(" ") + " " + v[insId].input;
    ...
    if (v[insId].equation.some((char) => SIGN.includes(char))) {
      ...
      if (v[insId].input.length !== 0) v[insId].equation.push(v[insId].input);
      else if (v[insId].equation.length === 0) v[insId].equation.push(0);
    }
    ...
    switch (val) {
      case "del":
        if (isNumeric(v[insId].equation[v[insId].equation.length - 1])) {
          ...
          v[insId].equation = [];
          v[insId].displayEquation = "";
        } else if (v[insId].input.length > 1)
          v[insId].input = v[insId].input.slice(0, v[insId].input.length - 1);
        else if (
          SIGN.includes(v[insId].equation[v[insId].equation.length - 1]) &&
          v[insId].input === ""
        )
          break;
        else v[insId].input = "0";
    ...
    if (v[insId].equation.length === 0 || insId === undefined) return;
