...
function ItemViewModel() {
  ...
  this.handleNumberClick = function (val) {
    if (val === "*-1") {
      ...
      this.displayEquation(
        this.equation().join(" ") + " negative(" + temp + ")"
      );
    } else if (val === ".") {
      this.input(this.input() + val);
    } else {
      if (this.eqLastIdx() === 0) this.equation.pop();
  ...
  this.handleSignClick = function (val) {
    if (this.equation().some((char) => SIGN.includes(char))) {
      ...
      if (this.input().length !== 0) this.equation.push(this.input());
      else if (this.eqLastIdx() === -1) this.equation.push(0);
    }
